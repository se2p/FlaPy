...
if [[ "$#" -lt 5 ]]; then
    ...
    INPUT_CSV is the flapy input csv file, which must have the following columns in the following order:
    ...
    FLAPY_ARGS can contain the following, but must always be provided, even as empty string. Must always be one string
        --random-order-seed
    OUT_DIR is the parent folder of the output results directory. If this option is not provided, the current directory is used
