...
# FORK FROM  https://github.com/cool-RR/GarlicSim-for-Python-3.x/blob/master/garlicsim_py3/garlicsim/general_misc/pickle_tools.py
...
def _is_type_atomically_pickleable(type_, thing=None):
    ...
    global i
    # print(f"IS PICKEABLE {i} {thing} {type_}")
    i = i + 1
    try:
    ...
    def get_result():
        ...
        if not hasattr(thing, "__class__") and (not isinstance(thing, type)):
            return False
...
class CutePickler(object):
    ...
    def __init__(self, file_, protocol=0):
        pickler = self.pickler = pickle._Pickler(file_, protocol)
        pickler.persistent_id = self.persistent_id
        self.dump, self.clear_memo = pickler.dump, pickler.clear_memo
    ...
    def persistent_id(self, obj):
        ...
        if passed_pre_filter and is_atomically_pickleable(obj):
            ...
            try:
                # address = address_tools.describe(obj)
                address = type(obj)
            except:
                address = "'No-Address-Found'"
            return "Filtered by pickle_tools (%s)" % address
...
class CuteUnpickler(object):
    ...
    def __init__(self, file_):
        unpickler = self.unpickler = pickle._Unpickler(file_)
    ...
    def persistent_load(self, id_string):
        ...
        if match:
            print(f"Match: {id_string}")
            description = match.groupdict()["description"]
            return FilteredObject(description)
        else:
            raise pickle.UnpicklingError(f'Invalid persistent id "{id_string}"')


def dumps_skip(obj: Any, protocol: int = None) -> bytes:
    f = io.BytesIO()
    CutePickler(f, protocol).dump(obj)
    res = f.getvalue()
